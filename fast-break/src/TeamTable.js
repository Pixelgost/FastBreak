          ...
          onClick={async (e) => {
            ...
            if (data.length === 40) {
              ...
              if (output.output.cpuData.length > 0) {
                ...
                const immediate_wins = Math.max(Math.min(Math.round(games * rate), games), 0);
                const immediate_loss = games - immediate_wins
                alert(`Predicted Record (End Of Seaons): ${wins} - ${losses}\nPredicted Record: ${immediate_wins} - ${immediate_loss}`);
              } else {
